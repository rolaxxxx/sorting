...
REAL4 coordinate_math::cross_prod(REAL4 vec_a, REAL4 vec_b){
    ...
    prod_rez[2]=vec_a[0]*vec_b[1]-vec_b[1]*vec_a[0];
    return prod_rez;
}
...
void coordinate_math::coordinate_math_(Dalele &particle, MAP_BOUNDS boundai)
{
    ...
    particle_coords[2]=(boundai.MAP_BOUNDS_MAX[2]-boundai.MAP_BOUNDS_MAX[2]/2);
    INT spind;
    ...
    further_coords[2]=(boundai.MAP_BOUNDS_MAX[2]-boundai.MAP_BOUNDS_MAX[2]/2);
    ...
    REAL4 vector_u, vector_v, vector_w, vector_a, vector_b, vector_c;
    vector_u=S1-S2/sqrt(pow(S1[0]-S2[0],2)+pow(S1[1]-S2[1],2)+pow(S1[2]-S1[2],2));
    vector_v=S1-S3/sqrt(pow(S1[0]-S3[0],2)+pow(S1[1]-S3[1],2)+pow(S1[2]-S3[2],2));

    cout << vector_u << " vektorius u reiksmes " << vector_v << " vektorius v reiksmes " << endl;
    //vector_w=-2*S1;
    //vector_w=S1*-2;
    vector_w=S1*-2;
    REAL a, b, c;
    ...
    a=(pow(S2[3],2)-pow(S1[3],2)+pow(S1[0],2)-pow(S2[0],2)+pow(S1[1],2)-pow(S2[1],2)+pow(S1[2],2)-pow(S2[2],2))
            /2*sqrt(pow(S1[0]-S2[0],2)+pow(S1[1]-S2[1],2)+pow(S1[2]-S1[2],2));
    ...
    vector_t=cross_prod(vector_u, vector_v)/sqrt(pow(Xproduktas[0],2)+pow(Xproduktas[1],2)+pow(Xproduktas[2],2));
   ...
   // cout << result_calc_a << " " << result_calc_b << endl;
    ...
    //begalybes problema jau cia
    result_calc_c_positive=1/2*(-dot_prod(vector_v, vector_t)+sqrt(pow(dot_prod(vector_w,vector_t),2)-4*result_calc_d));
    result_calc_c_negative=1/2*(-dot_prod(vector_v, vector_t)-sqrt(pow(dot_prod(vector_w,vector_t),2)-4*result_calc_d));

    cout << result_calc_c_negative << " " << result_calc_c_negative << endl;
