...
public:
  explicit ansicolor_sink(FILE *file) : target_file_(file) {
    should_do_colors_ =
        details::os::in_terminal(file) && details::os::is_color_terminal();
  ...
  ~ansicolor_sink() override { _flush(); }
...
protected:
  void _sink_it(const details::log_msg &msg) override {
    ...
    if (should_do_colors_) {
      ...
      fwrite(msg.formatted.data(), sizeof(char), msg.formatted.size(),
             target_file_);
  ...
  void _flush() override { fflush(target_file_); }
