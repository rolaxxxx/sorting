...
static const std::string days[]{"Sun", "Mon", "Tue", "Wed",
                                "Thu", "Fri", "Sat"};
...
// Full weekday name
static const std::string full_days[]{"Sunday",    "Monday",   "Tuesday",
...
class c_formatter SPDLOG_FINAL : public flag_formatter {
  void format(details::log_msg &msg, const std::tm &tm_time) override {
    msg.formatted << days[tm_time.tm_wday] << ' ' << months[tm_time.tm_mon]
                  << ' ' << tm_time.tm_mday << ' ';
...
class D_formatter SPDLOG_FINAL : public flag_formatter {
  void format(details::log_msg &msg, const std::tm &tm_time) override {
    pad_n_join(msg.formatted, tm_time.tm_mon + 1, tm_time.tm_mday,
               tm_time.tm_year % 100, '/');
  }
};
...
class full_formatter SPDLOG_FINAL : public flag_formatter {
  void format(details::log_msg &msg, const std::tm &tm_time) override {
    ...
    /* Slower version(while still very fast - about 3.2 million lines/sec under
    10 threads),
...
inline spdlog::pattern_formatter::pattern_formatter(
    ...
    : _eol(std::move(eol)), _pattern_time(pattern_time) {
  compile_pattern(pattern);
}

inline void
spdlog::pattern_formatter::compile_pattern(const std::string &pattern) {
  ...
  for (auto it = pattern.begin(); it != end; ++it) {
    ...
    {
      if (!user_chars)
        user_chars = std::unique_ptr<details::aggregate_formatter>(
