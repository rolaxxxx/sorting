...
public:
  ...
  std::shared_ptr<logger> create(const std::string &logger_name,
    ...
    if (_async_mode)
      new_logger = std::make_shared<async_logger>(
          logger_name, sinks_begin, sinks_end, _async_q_size, _overflow_policy,
          _worker_warmup_cb, _flush_interval_ms, _worker_teardown_cb);
    else
      new_logger =
          std::make_shared<logger>(logger_name, sinks_begin, sinks_end);
  ...
  create_async(const std::string &logger_name, size_t queue_size,
    ...
    auto new_logger = std::make_shared<async_logger>(
        logger_name, sinks_begin, sinks_end, queue_size, overflow_policy,
  ...
  create_async(const std::string &logger_name, size_t queue_size,
               ...
               const std::function<void()> &worker_teardown_cb,
               sinks_init_list sinks) {
    return create_async(logger_name, queue_size, overflow_policy,
                        worker_warmup_cb, flush_interval_ms, worker_teardown_cb,
