...
__kernel void count(__global uint *A, __global uint *Index,  __global uint *B, int n) {
/*			int b_id = get_group_id(0);
        ...
        for (int i = offset; i < offset + t_size && i < MAX_VALUE; ++i)
                count[i] = 0; // ar galima keisti sito masyvo reiksmes rusiuojant by key 
