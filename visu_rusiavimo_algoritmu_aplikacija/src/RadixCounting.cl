...
// Read radix groups in offset by one in the GRF so a zero can be inserted at the beginning
    ...
    {
        sRadixSum[pos] += sum;
	//printf("pos sRadixSum  reiksme %d " , sRadixSum[pos] );
	pos++;
    ...
    if (get_local_id(0) == 0) {
        dRadixBlockSum[get_group_id(0)] = sRadixSum[PREFIX_NUM_THREADS_PER_BLOCK * (PREFIX_BLOCKSIZE / PREFIX_NUM_THREADS_PER_BLOCK + 1) - 1];
        dRadixSum[get_group_id(0) * PREFIX_BLOCKSIZE] = 0;
}
...
__kernel void RadixAddOffsetsAndShuffle(__global uint* pKeys, __global uint* pVals, __global uint* outDeviceKeys, __global uint* outDeviceVals, uint elements, uint elements_rounded_to_3072, uint shift, __global uint* gRadixSum, __global uint* dRadixSum, __global uint* gRadixBlockSum, __global uint* dRadixBlockSum, __global uint* sRadixSum)
{
...
// Read offsets from previous blocks
    ...
    while (pos < end )
    {
        ...
        uint ppos = p + tpos;
	//printf("ppos sRadixSum  reiksme %d " , sRadixSum[ppos] );
