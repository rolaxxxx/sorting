...
public:
  ...
  static filename_t calc_filename(const filename_t &filename,
    ...
    typename std::conditional<std::is_same<filename_t::value_type, char>::value,
                              fmt::MemoryWriter, fmt::WMemoryWriter>::type w;
    if (index != 0u) {
  ...
  void _rotate() {
    ...
    for (auto i = _max_files; i > 0; --i) {
      ...
      if (details::file_helper::file_exists(src) &&
          details::os::rename(src, target) != 0) {
...
struct default_daily_file_name_calculator {
  ...
  static filename_t calc_filename(const filename_t &filename) {
    ...
    w.write(SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}-{:02d}{}"),
            basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
            tm.tm_min, ext);
    return w.str();
  }
};
...
struct dateonly_daily_file_name_calculator {
  ...
  static filename_t calc_filename(const filename_t &filename) {
    ...
    std::conditional<std::is_same<filename_t::value_type, char>::value,
                     fmt::MemoryWriter, fmt::WMemoryWriter>::type w;
    w.write(SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}{}"), basename,
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, ext);
    return w.str();
  }
};
...
public:
  ...
  daily_file_sink(filename_t base_filename, int rotation_hour,
                  int rotation_minute)
      : _base_filename(std::move(base_filename)), _rotation_h(rotation_hour),
