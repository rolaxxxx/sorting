...
class async_log_helper {
  ...
  struct async_msg {
    ...
    explicit async_msg(const details::log_msg &m)
        : level(m.level), time(m.time), thread_id(m.thread_id),
          txt(m.raw.data(), m.raw.size()), msg_type(async_msg_type::log),
          msg_id(m.msg_id) {
...
public:
  ...
  async_log_helper(formatter_ptr formatter, std::vector<sink_ptr> sinks,
                   size_t queue_size, const log_err_handler err_handler,
...
private:
  ...
  // pop next message from the queue and process it. will set the last_pop to
  // the pop time
  ...
  // sleep,yield or return immediately using the time passed since last message
  // as a hint
...
inline spdlog::details::async_log_helper::async_log_helper(
    formatter_ptr formatter, std::vector<sink_ptr> sinks, size_t queue_size,
    log_err_handler err_handler, const async_overflow_policy overflow_policy,
    std::function<void()> worker_warmup_cb,
    const std::chrono::milliseconds &flush_interval_ms,
    std::function<void()> worker_teardown_cb)
    : _formatter(std::move(formatter)), _sinks(std::move(sinks)),
      _q(queue_size), _err_handler(std::move(err_handler)),
      _flush_requested(false), _terminate_requested(false),
...
// Try to push and block until succeeded (if the policy is not to discard when
// the queue is full)
inline void
...
inline void spdlog::details::async_log_helper::push_msg(
    details::async_log_helper::async_msg &&new_msg) {
...
inline bool spdlog::details::async_log_helper::process_next_msg(
  ...
  // Handle empty queue..
  // This is the only place where the queue can terminate or flush to avoid
  // losing messages already in the queue
...
inline void spdlog::details::async_log_helper::handle_flush_interval(
  ...
  auto should_flush =
      _flush_requested ||
      (_flush_interval_ms != std::chrono::milliseconds::zero() &&
...
inline void spdlog::details::async_log_helper::set_error_handler(
