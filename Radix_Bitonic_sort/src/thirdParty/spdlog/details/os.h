...
#if defined(SPDLOG_DISABLE_TID_CACHING) ||                                     \
    (defined(_MSC_VER) && (_MSC_VER < 1900)) || defined(__cplusplus_winrt) ||  \
...
#else // gnu version (might not use the given buf, so its retval pointer must be
      // used)
  auto err = strerror_r(err_num, buf, buf_size); // let compiler choose type
  return errno_to_string(
      buf, err); // use overloading to select correct stringify function
#endif
}
